...
namespace Widget {
	...
	struct DragFloat {
		static void Display(const char* label, float* data, float width, float step = 1.0f, float min = 0.0f, float max = 0.0f) {
	...
	struct DragVec3 {
		static void Display(std::string label, glm::vec3& v, ImTextureID icon = 0, float step = 1.0f, float min = 0.0f, float max = 0.0f) {
			ImGui::Image(icon, ImVec2(15.0f, 15.0f), ImVec2(0, 1), ImVec2(1, 0));
			ImGui::SameLine();
			ImGui::Text(label.c_str());
			ImGui::SameLine();
			std::string vx = "##X" + label;
			DragFloat::Display(vx.c_str(), &v.x, 50.0f, step, min, max); ImGui::SameLine(); ImGui::Text("X");
			ImGui::SameLine();
			std::string vy = "##Y" + label;
			DragFloat::Display(vy.c_str(), &v.y, 50.0f, step, min, max); ImGui::SameLine(); ImGui::Text("Y");
			ImGui::SameLine();
			std::string vz = "##Z" + label;
			DragFloat::Display(vz.c_str(), &v.z, 50.0f, step, min, max); ImGui::SameLine(); ImGui::Text("Z");
		}
	};
	...
	struct EntitySelectable {
		static int Display(const char* label, int entity, int curr, ImTextureID icon) {
			ImGui::Image(icon, ImVec2(15.0f, 15.0f), ImVec2(0, 1), ImVec2(1, 0));
			ImGui::SameLine();
			if (ImGui::Selectable(label, curr == entity)) { curr = entity; }
			return curr;
		}
	};

	struct ToolButton {
		static bool Display(ImTextureID icon, float size = 25.0f) {
